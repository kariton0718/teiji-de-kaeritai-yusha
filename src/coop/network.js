...
export class CoopLink{
 constructor(Peer,{now=()=>performance.now(),onStatus=()=>{},onMessage=()=>{},onClosed=()=>{}}={}){this.Peer=Peer;this.now=now;this.onStatus=onStatus;this.onMessage=onMessage;this.onClosed=onClosed;this.peer=null;this.conn=null;this.role=null;this.closed=false;this.connected=false;this.lastSeen=0;this.lastPing=0;this.lastSeq=-1;this.seq=0;this.deadline=0;this.started=false;}
 open(invite=null){this.role=invite?'guest':'host';this.key=invite?.key||randomKey();this.room=invite?.room||`teiji-${randomKey()}`;this.deadline=this.now()+20000;this.peer=new this.Peer(this.role==='host'?this.room:undefined,{debug:0,config:{iceServers:[{urls:'stun:stun.l.google.com:19302'}]}});
  this.peer.on('open',()=>{if(this.closed)return;if(this.role==='host'){this.deadline=0;this.onStatus('waiting');}else this.attach(this.peer.connect(this.room,{reliable:true,serialization:'json',metadata:{v:COOP_VERSION,key:this.key}}));});
  this.peer.on('connection',conn=>{if(this.closed||this.role!=='host'||this.conn||conn.metadata?.v!==COOP_VERSION||conn.metadata?.key!==this.key){conn.on('open',()=>{conn.send({v:COOP_VERSION,type:'reject'});conn.close();});return;}this.deadline=this.now()+15000;this.attach(conn);});
  this.peer.on('error',error=>{if(error.type==='peer-unavailable')this.fail('部屋が見つかりません。ホストが部屋を開いているか確認してください。');else this.fail('接続できませんでした。回線を変えるか、部屋を作り直してください。');});
  this.peer.on('disconnected',()=>{if(!this.connected)this.fail('接続サービスとの通信が切れました。部屋を作り直してください。');});
 ...
 poll(){if(this.closed)return;const now=this.now();if(this.deadline&&now>this.deadline){this.fail('接続がタイムアウトしました。Wi-Fiや携帯回線を切り替えてお試しください。');return;}if(this.connected){if(now-this.lastSeen>6000){this.fail('相手の応答がなくなりました。部屋を作り直してください。');return;}if(now-this.lastPing>1000){this.lastPing=now;this.send('ping');}}}
