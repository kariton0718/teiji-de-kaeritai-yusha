...
export class Game{
  ...
  bossStats(e){const d=C.rankBosses[e.rankIndex];return e.bossPhase>=2&&d.secondForm?{...d,...d.secondForm}:d;}
  ...
  updateRankBoss(e,dt){
    const d=this.bossStats(e),phase=Math.min(d.phases,1+Math.floor((1-e.hp/e.maxHp)*d.phases));if(phase!==e.bossPhase){e.bossPhase=phase;e.pattern=0;e.phaseSeen=new Set();e.left=Math.min(e.left,.2);const surge=C.bossAttack.phaseReinforcements[phase-1]||0;if(surge)this.scheduleBossMinions(surge,'bossPhase');this.banner={title:`魔王${d.rank}・第${phase}形態`,detail:e.rankIndex===6?'監査レベル上昇！ 高速連続攻撃！':phase===d.phases&&d.phases===3?'玉座機械、全開！ 攻撃速度上昇！':'増援とともに攻撃速度上昇！',left:1.5};this.events.push('bossPhase');if(d.secondForm&&phase===2){e.state='phaseShift';e.left=d.secondForm.transition;e.weak=false;e.attackKind=null;e.chargesLeft=0;e.chainCharge=false;e.moveTarget=null;e.moveFrom=null;this.enemyProjectiles=[];this.enemyAreas=[];this.banner={title:`第2形態・${d.secondForm.name}`,detail:d.secondForm.detail,left:2.8};this.notice=d.secondForm.detail;return;}}
    e.report.time+=dt;e.weak=e.state==='recover';if(e.state==='fastMove'){this.updateFastMove(e,dt);return;}e.left-=dt;if(e.state==='phaseShift'){if(e.left<=0)e.state='chase';return;}if(e.state==='recover'){if(e.left<=0){e.state='chase';e.left=Math.max(.08,this.bossTempo(e,d)-C.bossAttack.recovery);}return;}if(e.state==='attackWarn'){if(e.left<=0)this.executeRankAttack(e);return;}if(e.state==='charge'){const x=e.pos.x+e.dir.x*(d.chargeSpeed||C.bossAttack.chargeSpeed)*dt,y=e.pos.y+e.dir.y*(d.chargeSpeed||C.bossAttack.chargeSpeed)*dt;if(this.isBlocked(x,y,e.radius)){e.state='recover';e.left=C.bossAttack.recovery+0.45;this.notice='障害物に激突！ 反撃時間！';this.events.push('stun');return;}this.nav.move(e.pos,e.dir.x*(d.chargeSpeed||C.bossAttack.chargeSpeed)*dt,e.dir.y*(d.chargeSpeed||C.bossAttack.chargeSpeed)*dt,e.radius);this.contactHero(e,d.damage);if(e.left<=0){if(e.chargesLeft>1){e.chargesLeft--;e.chainCharge=true;e.state='attackWarn';e.left=.56;e.dir=norm(this.hero.x-e.pos.x,this.hero.y-e.pos.y);}else{e.state='recover';e.left=C.bossAttack.recovery;}}return;}
  ...
  startRankAttack(e){e.chainCharge=false;const d=this.bossStats(e),patterns=d.patterns[e.bossPhase-1],kind=patterns[e.pattern++%patterns.length];e.attackKind=kind;e.state='attackWarn';const base=kind==='charge'||kind==='multi'?(d.chargeWarning||C.bossAttack.warning):C.bossAttack.warning;e.left=this.bossWarning(e,base);e.dir=norm(this.hero.x-e.pos.x,this.hero.y-e.pos.y);if(['sidestep','jumpSlam','copyShift','flank','presidentRush'].includes(kind))e.moveTarget=this.findBossTarget(e,kind);e.phaseSeen.add(kind);e.report.attacks++;this.notice=`魔王${d.rank}：${{droneBarrage:'ドローン連続爆撃',charge:'突進',summon:'増援の号令',slam:'連続ハンコ',shockwave:'衝撃波',fan:'扇状書類弾',clone:'コピー分身',sweep:'定規薙ぎ払い',multi:'連続突進',crossLaser:'交差レーザー',surround:'包囲射撃',beam:'極太レーザー',floor:'連続床攻撃',reorg:'組織再編',sidestep:'横移動から突進',jumpSlam:'予告地点へハンコジャンプ',copyShift:'分身を残す高速移動',flank:'外周へ移動して挟み撃ち',presidentRush:'高速移動から連続攻撃'}[kind]}！`;this.events.push('warn');}
  executeRankAttack(e){const d=this.bossStats(e),kind=e.attackKind;e.state='recover';e.left=C.bossAttack.recovery;
