...
function menu(){
  ...
  else if(game.state==='bossIntro'){const d=C.rankBosses[game.stage-1];$('badge').textContent=`STAGE ${game.stage} BOSS / ${d.rank}`;$('heading').textContent=`魔王${d.rank}`;$('message').textContent=`${['増援の号令と予告突進','連続ハンコと広がる衝撃波','コピー分身と扇状書類弾','定規薙ぎ払いと連続突進','交差レーザーと包囲射撃','玉座機械の極太レーザー・弾幕・連続床'][game.stage-1]}\n予告を避け、金色の反撃時間に攻撃。`;$('action').textContent='ボス戦開始';$('hint').textContent=game.stage===6?'撃破後、退勤ゲートへ':'撃破後、新技を習得';}
...
function ui(){
  ...
  let status=game.notice||'出現予告から離れて敵を全滅させよう';if(game.state==='playing'&&game.phase==='wave')status=`敵出現中　残り${remaining}体`;if(game.state==='playing'&&game.boss?.weak)status=`反撃時間 ${Math.max(0,game.boss.left).toFixed(1)}秒：攻撃のチャンス！`;$('status').textContent=status;$('status').className='status '+(h.hurtFlash>0?'danger':game.phase==='escape'?'clear':'');
