...
export function clearLine(a,b,obstacles=solids,radius=CONFIG.radius){
  const n=Math.ceil(Math.hypot(b.x-a.x,b.y-a.y)/4);
  for(let i=0;i<=n;i++){const t=n?i/n:0;if(blocked(a.x+(b.x-a.x)*t,a.y+(b.y-a.y)*t,radius,obstacles))return false;}
  return true;
}
...
export function move(body,dx,dy,obstacles=solids,radius=CONFIG.radius){
  if(!blocked(body.x+dx,body.y,radius,obstacles))body.x+=dx;
  if(!blocked(body.x,body.y+dy,radius,obstacles))body.y+=dy;
}
export function navigation(obstacles,width=CONFIG.width,height=CONFIG.height){
  return {
    ...
    clearLine:(a,b,radius=CONFIG.radius)=>clearLine(a,b,obstacles,radius),
    findPath:(a,b)=>findPath(a,b,obstacles,width,height),
    move:(p,x,y,radius=CONFIG.radius)=>move(p,x,y,obstacles,radius)
  };
}
