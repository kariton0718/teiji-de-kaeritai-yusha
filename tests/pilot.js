...
export function pilot(g){
  ...
  if(h.energy<62&&h.shields>0&&h.shieldLeft===0)input.shield=true;
  ...
  if(floor&&floor.left<.62){Object.assign(input,unit(h.x-floor.pos.x,h.y-floor.pos.y));input.dash=h.dashCd===0;return input;}
  ...
  if(boss&&['chargeWarn','multiWarn'].includes(boss.state)){Object.assign(input,unit(-boss.dir.y,boss.dir.x));input.dash=h.dashCd===0;return input;}
  ...
  if(brute){Object.assign(input,unit(h.x-brute.pos.x,h.y-brute.pos.y));input.dash=h.dashCd===0;return input;}
