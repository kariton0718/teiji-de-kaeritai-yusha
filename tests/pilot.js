...
export function pilot(g){
  const h=g.hero,input={x:0,y:0,attack:false,ultimate:false};
  ...
  const floor=g.enemyAreas.filter(a=>!a.fired&&Math.hypot(h.x-a.pos.x,h.y-a.pos.y)<a.radius+18).sort((a,b)=>a.left-b.left)[0];
  if(floor&&floor.left<.82){Object.assign(input,unit(h.x-floor.pos.x,h.y-floor.pos.y));return input;}
  if(boss?.state==='meetingWarn'&&Math.hypot(h.x-boss.pos.x,h.y-boss.pos.y)<C.boss.meetingRadius+25){Object.assign(input,unit(h.x-boss.pos.x,h.y-boss.pos.y));return input;}
  if(boss&&['chargeWarn','multiWarn'].includes(boss.state)){Object.assign(input,unit(-boss.dir.y,boss.dir.x));return input;}
  const ghost=g.activeEnemies.find(e=>e.type==='ghost'&&e.state==='warn'&&Math.abs((h.x-e.pos.x)*e.dir.y-(h.y-e.pos.y)*e.dir.x)<35);
  if(ghost){Object.assign(input,unit(-ghost.dir.y,ghost.dir.x));return input;}
  const brute=g.activeEnemies.find(e=>e.type==='brute'&&e.state==='warn'&&Math.hypot(h.x-e.pos.x,h.y-e.pos.y)<C.enemies.brute.areaRadius+20);
  if(brute){Object.assign(input,unit(h.x-brute.pos.x,h.y-brute.pos.y));return input;}
  const shot=g.enemyProjectiles.find(p=>Math.hypot(h.x-p.pos.x,h.y-p.pos.y)<80);
  if(shot){Object.assign(input,unit(-shot.dir.y,shot.dir.x));return input;}
